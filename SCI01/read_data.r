...
cor(airdata$sent_message, airdata$sent_booking_request) # 0.21

# What are the chances that one will send the message if searches a property ?
cor(airdata$did_search, airdata$sent_message) # 0.04


# Does someone's staying more on site means increased chances of booking ? 
cor(as.numeric(airdata$session_lengths), airdata$sent_booking_request, use="complete") # 0.15